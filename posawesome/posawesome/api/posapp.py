...
#     # # إعادة حساب الضرائب والمبالغ قبل الحفظ
...
def update_invoice(data):
    ...
    # معالجة الفواتير المرتجعة
    if invoice_doc.is_return and invoice_doc.return_against:
        ref_doc = frappe.get_cached_doc("Sales Invoice", invoice_doc.return_against)
        
        # التحقق من تطابق العناصر
        for item in invoice_doc.items:
            matching_item = next(
                (i for i in ref_doc.items if i.item_code == item.item_code), None
            )
            if not matching_item:
                frappe.throw(
                    _("Row #{0}: Returned Item {1} does not exist in Sales Invoice {2}").format(
                        item.idx, item.item_code, invoice_doc.return_against
                    )
                )
    ...
    # تحديث حالة فاتورة الشراء الأصلية
    if invoice_doc.is_return and invoice_doc.return_against:
        frappe.db.set_value(
            "Sales Invoice",
